...
public class IntegrationTests
{
    ...
    private const string Message =
        "This scuffed test relies on the Test.Mod source being in a certain spot relative to this test in source.";
    ...
    {
        var sourceFolder = GetSource();
        foreach (var file in Directory.EnumerateFiles(sourceFolder, "*.txt")) yield return new object[] { file };
    }
    ...
    private static string GetSource([CallerFilePath] string thisSourceFile = null)
    {
        ...
        var receivedFolder = Path.Combine(directory, PathToPluginTest);
